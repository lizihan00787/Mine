#include<bits/stdc++.h>
#define int long long
#define re register
#define PII pair<int,int>
#define fi first
#define se second
#define MOD 998244353
#define io() ios::sync_with_stdio(false),cin.tie(NULL),cout.tie(NULL)
#define mk(a,b) make_pair(a,b)
#define pqd priority_queue<int,vector<int>,less<int> >
#define pqx priority_queue<int,vector<int>,greater<int> >
using namespace std;
template<typename P>
inline void read(P &x){
    P res=0,f=1;
    char ch=getchar();
    while(ch<'0' || ch>'9'){
        if(ch=='-') f=-1;
        ch=getchar();
    }
    while(ch>='0' && ch<='9'){
        res=res*10+ch-'0';
        ch=getchar();
    }
    x=res*f;
}
int T=1;
#define lson (rt<<1)
#define rson (rt<<1|1)
const int N=1e5+10;
int n,q,a[N<<2];
struct edge{
    int to,nxt,u;
}e[N<<1];
int cnt=0,head[N];
int siz[N],son[N],fa[N],dep[N],top[N];
int dfsx[N],pre[N],tot=0;
void add(int u,int v){
    e[++cnt].u=u;
    e[cnt].nxt=head[u];
    e[cnt].to=v;
    head[u]=cnt;
    e[++cnt].u=v;
    e[cnt].nxt=head[v];
    e[cnt].to=u;
    head[v]=cnt;
}
void dfs1(int u,int f){
    siz[u]=1;
    for(int i=head[u];i;i=e[i].nxt){
        int to=e[i].to;
        if(to==f) continue;
        dep[to]=dep[u]+1;
        fa[to]=u;
        dfs1(to,u);
        siz[u]+=siz[to];
        if(siz[to]>siz[son[u]]) son[u]=to;
    }
}
void dfs2(int u,int tp){
    dfsx[u]=++tot;pre[tot]=u,top[u]=tp;
    if(son[u]) dfs2(son[u],tp);
    for(int i=head[u];i;i=e[i].nxt){
        int to=e[i].to;
        if(to==fa[u] || to==son[u]) continue;
        dfs2(to,to);
    }
}
struct Stree{
    struct node{
        int maxx,sum;
    }t[N<<2];
    inline void pushup(int rt){
            t[rt].sum=t[lson].sum+t[rson].sum;
            t[rt].maxx=max(t[lson].maxx,t[rson].maxx);
    }
    inline void build(int rt,int l,int r){
        if(l==r){
            t[rt].sum=a[pre[l]];
            t[rt].maxx=a[pre[l]];
            return;
        }
        int mid=(l+r)>>1;
        build(lson,l,mid);
        build(rson,mid+1,r);
        pushup(rt);
    }
    inline void update(int rt,int l,int r,int k,int x){
        if(l==r){
            t[rt].sum=x;
            t[rt].maxx=x;
            return;
        }
        int mid=(l+r)>>1;
        if(k<=mid) update(lson,l,mid,k,x);
        else update(rson,mid+1,r,k,x);
        pushup(rt);
    }
    inline int querysum(int rt,int l,int r,int L,int R){
        if(L<=l && r<=R) return t[rt].sum;
        int mid=(l+r)>>1;
        int ans=0;
        if(L<=mid) ans+=querysum(lson,l,mid,L,R);
        if(R>mid) ans+=querysum(rson,mid+1,r,L,R);
        pushup(rt);
        return ans;
    }
    
    inline int querymax(int rt,int l,int r,int L,int R){
        if(L<=l && r<=R) return t[rt].maxx;
        int mid=(l+r)>>1;
        int ans=-0x3f3f3f3f3f3f3f3f;
        if(L<=mid) ans=max(querymax(lson,l,mid,L,R),ans);
        if(R>mid) ans=max(querymax(rson,mid+1,r,L,R),ans);
        pushup(rt);
        return ans;
    }
}Q;
int qsum(int u,int v){
    int ans=0;
    while(top[u]!=top[v]){
        if(dep[top[u]]<dep[top[v]]) swap(u,v);
        ans+=Q.querysum(1,1,n,dfsx[top[u]],dfsx[u]);
        u=fa[top[u]];
    }
    if(dep[u]<dep[v]) swap(u,v);
    ans+=Q.querysum(1,1,n,dfsx[v],dfsx[u]);
    return ans;
}
int qmax(int u,int v){
    int ans=-0x3f3f3f3f3f3f3f3f;
    while(top[u]!=top[v]){
        if(dep[top[u]]<dep[top[v]]) swap(u,v);
        ans=max(ans,Q.querymax(1,1,n,dfsx[top[u]],dfsx[u]));
        u=fa[top[u]];
    }
    if(dep[u]<dep[v]) swap(u,v);
    ans=max(ans,Q.querymax(1,1,n,dfsx[v],dfsx[u]));
    return ans;
}
signed main(){
    auto solve=[&](){
        read(n);
        for(int i=1;i<n;++i){
            int u,v;
            read(u),read(v);
            add(u,v);
        }
        for(int i=1;i<=n;++i) read(a[i]);
        dep[1]=1;fa[1]=1;
        dfs1(1,-1),dfs2(1,1);
        Q.build(1,1,n);
        read(q);
        for(int i=1;i<=q;++i){
            char op[10];
            cin>>op;
            if(op[0]=='C'){
                int k,x;
                read(k),read(x);
                Q.update(1,1,n,dfsx[k],x);
            }
            else{
                if(op[1]=='S'){
                    int u,v;
                    read(u),read(v);
                    cout<<qsum(u,v)<<endl;
                }
                else{
                    int u,v;
                    read(u),read(v);
                    cout<<qmax(u,v)<<endl;
                }
            }
        }
    };
    //io();
    //read(T);
    while(T--) solve();
    return 0;
}