...
struct block{
    ...
    int ask(int l,int r,int x){
        ...
        for(int i=lb+1;i<rb;++i) res+=lower_bound(b+L[i],b+R[i]+1,x-kuai[i])-b-L[i];
        for(int i=l;i<=min(r,R[lb]);++i) res+=a[i]+kuai[lb]<x;
        if(lb<rb) for(int i=L[rb];i<=r;++i) res+=a[i]+kuai[rb]<x;
        return res;
    }
}blk;
...
signed main(){
    auto solve=[&](){
        ...
        for(int i=1;i<=n;++i){
            ...
            else cout<<blk.ask(l,r,x*x)<<endl;
        }
    };
    //freopen(".in","r",stdin);
